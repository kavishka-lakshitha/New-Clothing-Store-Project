...
import kidsImage from "../../Images/home3.jpg";
import { useCart } from "../../context/CartContext";

export default function KidsTShirts() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/api/kids?sub=T-Shirts")
  ...
  const handleAddToCart = (item) => {
    addToCart({
      ...
      image: item.imageUrl ? `http://localhost:8080/images/${item.imageUrl}` : kidsImage,
    });
    setAddedId(item.id);
  ...
  return (
    <div className="men">
      <div className="men__hero" style={{ backgroundImage: `url(${kidsImage})`, backgroundSize: "cover", backgroundPosition: "center" }}>
        <p className="men__hero-label">Amandi Fashion</p>
        <h1 className="men__hero-title">Kids T-Shirts</h1>
        <p className="men__hero-desc">Cool & comfy tees for kids</p>
      </div>
              ...
              return (
                <div key={item.id} className="men__card">
                  <div className="men__card__img-wrap">
                    <img
                      src={item.imageUrl ? `http://localhost:8080/images/${item.imageUrl}` : kidsImage}
                      alt={item.name}
                      onError={(e) => (e.target.src = kidsImage)}
                    />
