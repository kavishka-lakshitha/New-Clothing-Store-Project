...
export default function AccessoriesBags() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/api/accessories?sub=Bags")
  ...
  return (
    <div className="men">
      <div className="men__hero" style={{ backgroundImage: `url(${accessoriesImage})`, backgroundSize: "cover", backgroundPosition: "center" }}>
        ...
        <h1 className="men__hero-title">Bags</h1>
        <p className="men__hero-desc">Stylish bags for every occasion</p>
      </div>
              ...
              return (
                <div key={item.id} className="men__card">
                  <div className="men__card__img-wrap">
                    <img
                      src={item.imageUrl ? `http://localhost:8080/images/${item.imageUrl}` : accessoriesImage}
                      alt={item.name}
